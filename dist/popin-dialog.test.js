...
suite('popin-dialog', () => {
    ...
    test('hides when lose focus', async () => {
        const el = await fixture(html `<popin-dialog><head>Test</head></popin-dialog>`);
        el.focus();
        el.blur();
    ...
    test('do not hide when lose focus with autoclose set to false', async () => {
        const el = await fixture(html `<popin-dialog no-auto-close><head>Test</head></popin-dialog>`);
        el.focus();
        el.blur();
        await new Promise(resolve => setTimeout(resolve, 0));
...
//# sourceMappingURL=popin-dialog.test.js.map